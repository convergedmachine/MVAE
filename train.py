...
class FreyFaceDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        if not self._check_exists():
            raise RuntimeError(
                'Dataset do not found in the directory \"{}\". \nYou can download FreyFace '
...
def save_curves(curves_dir: str, dataset: str, model: str, latent: int, seed: int,
    ...
    """Save training curves (ELBO and MSE) to JSON.
    ...
    8) train_mse_hist: list of training MSE values per epoch
    9) test_mse_hist: list of test MSE values per epoch
    """
...
def gather_elbo_mse(model, loader, device, input_size):
    ...
    Returns:
        dict with:
          - 'elbo': mean ELBO per batch (higher is better)
          - 'mse_per_pixel': mean MSE per pixel (lower is better)
    """
    model.eval()
    total_elbo = 0.0
    total_mse = 0.0
...
def evaluate_model(model, latent, train_loader, test_loader, device,
                   input_size: int, results_dir: str,
                   model_name: str, dataset_name: str, seed: int):
    ...
    clf = LogisticRegression(
        ...
        multi_class="multinomial", n_jobs=-1, verbose=0
    ).fit(Ztr, Ytr)
    ...
    out_path = os.path.join(
        results_dir,
        f"{dataset_name}_{model_name}_d{latent}_s{seed}_metrics.json"
    )
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Early stopping patience (epochs without validation ELBO improvement)."
    )
    p.add_argument(
        "--min-delta", type=float, default=0.0,
        help="Minimum ELBO improvement to reset early stopping patience."
    )
    ...
    p.add_argument(
        ...
        help="Fraction of data reserved for testing/validation."
    )

    # ---- Evaluation Mode ----
    p.add_argument(
        "--evaluate", action="store_true",
        help=("If set, run evaluation after training using the "
...
def load_dataset(name: str, root: str = "data_folder"):
    ...
    Notes:
      - Torchvision datasets are concatenated (train+test), flattened, and normalized to [0,1].
    ...
    if name == "freyface":
        entire_dataset = FreyFaceDataset(root=root + '/FreyFace',
...
def build_dataloaders(args):
    ...
    - Returns: train_loader, test_loader, input_size, dataset_name
      where input_size is int D (flattened).
    """
    import numpy as np
    import torch
...
def main():
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_idx, (batch_x, _) in enumerate(train_loader):
            ...
            # model.compute_loss is ELBO (higher is better), so we negate for minimization
            ...
            if (batch_idx + 1) % 100 == 0:
                print(
                    ...
                    f"Batch {batch_idx + 1}, "
                    f"Aver_Loss: {aver_loss.item():.4f}"
                )

        # ----- End-of-epoch evaluation (train + validation) -----
        model.eval()
        with torch.no_grad():
        ...
        # ----- Early stopping uses validation ELBO (here: test_loader) -----
        val_elbo = test_elbo_epoch  # treat test_loader as validation split

        improved = val_elbo > best_val_elbo + min_delta
        if improved:
            ...
            epochs_no_improve = 0
            print(
                f"[early-stopping] Epoch {epoch}: new best val ELBO = {best_val_elbo:.6f}"
            )
        else:
            epochs_no_improve += 1
            print(
                f"[early-stopping] Epoch {epoch}: no improvement "
                f"({epochs_no_improve}/{patience})"
            )
            if epochs_no_improve >= patience:
                print(
                    f"[early-stopping] Patience reached. "
                    f"Stopping at epoch {epoch}. Best epoch: {best_epoch}, "
                    f"best val ELBO: {best_val_elbo:.6f}"
                )
                break
    ...
    if best_state_dict is not None:
        ...
        print(
            f"[early-stopping] Loaded best model from epoch {best_epoch} "
            f"(val ELBO = {best_val_elbo:.6f})"
        )
    else:
        print("[early-stopping] WARNING: best_state_dict is None, using final epoch model.")

    # ----- Save best checkpoint -----
    os.makedirs(args.params_dir, exist_ok=True)
    ckpt_path = os.path.join(
        args.params_dir,
        f"{dataset_name}_zdim{latent_size}_{args.model}.pkl"
    )
    torch.save(model.state_dict(), ckpt_path)
    print(f"[checkpoint] Saved best model to: {ckpt_path}")

    # ----- Save curves (correctly: ELBO vs MSE) -----
    save_curves(
        curves_dir,
        ...
        latent=latent_size,
        seed=args.seed,
        train_elbo_hist=train_elbo_curve,
        test_elbo_hist=test_elbo_curve,
        train_mse_hist=train_msepp_curve,
        test_mse_hist=test_msepp_curve
    )

    # ----- Optional evaluation with the best validation model -----
    if args.evaluate:
        _ = evaluate_model(
            model=model,
            latent=latent_size,
